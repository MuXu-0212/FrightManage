...
//用户订票
int vip_buy_strick(p_userinfo u, p_flight f, int flight_frequency)
{
	fflush(stdin);
	char input = 0;
...
//连续购票
int vip_buy_strick_continue(p_userinfo u, p_flight f, int flight_frequency)
{
	int input=0;
	vip_buy_strick( u, f, flight_frequency);
	while (1)
	{
		printf("\n\n1.继续购票  0.退出\n");
		printf(">");
...
//用户购票功能
int vip_buy(p_userinfo u, p_flight f)
{
	system("clear");
	int input = 0;
...
//用户修改票数功能
int vip_revise_strick(p_userinfo u)
{
	if (u->next == NULL)
	{
		printf("\n\n表空!\n\n");
...
int vip(p_userinfo u, p_flight f)
{
	int input=0;
	while (1)
	{
